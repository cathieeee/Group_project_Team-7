...
def get_metadata(output_fn, input_fn, key_file):
    HYDRATION_MODE = ""
    ...
    if '.tsv' in input_fn:
        inputfile_data = pd.read_csv(input_fn, sep='\t')
        print('tab seperated file, using \\t delimiter')
    elif '.csv' in input_fn:
        inputfile_data = pd.read_csv(input_fn)
    elif '.txt' in input_fn:
    ...
    last_tweet = None
    if osp.isfile(output_fn) and osp.getsize(output_fn) > 0:
